<script>
    import { GOLD, SILVER, PLATIN } from '../../constants/membership-colors';
...
<div class="flex my-2 items-center userItem relative" on:mouseover={handelHover} on:focus={handelHover}>
    <div class="flex userItem_avatar">
        <div
            class=" {item.membership === GOLD
                ? 'bg-yellow-400'
                : item.membership === SILVER
                ? 'bg-gray-400'
                : item.membership === PLATIN
                ? 'bg-purple-400'
                : 'bg-white-300'} avatar-membership-cicle"
        />
        <img class=" m-px rounded-full userItem-photo" src="./../../static/small/{item.avatar}" alt="{item.avatar} " />
        <div class=" {item.active === ACTIVE ? 'active-avatar' : 'inactive-avatar'}" />
    </div>
